#!/bin/bash
#SBATCH --job-name eval-knn-gender-celeba-blurred-a-with-blurred-dataset
#SBATCH --output eval-knn-gender-celeba-blurred-a-with-blurred-dataset-%j.out
...
echo "$(date)"
echo "Starting CelebA_blurred_A knn gender evaluation with blurred train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir CelebA_blurred_A/eval/training_124999/knn_gender_with_blurred_dataset \

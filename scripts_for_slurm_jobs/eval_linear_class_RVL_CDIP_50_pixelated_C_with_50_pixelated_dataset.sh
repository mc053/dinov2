#!/bin/bash
#SBATCH --job-name eval-linear-class-rvl-cdip-50-pixelated-c-with-50-pixelated-dataset
#SBATCH --output eval-linear-class-rvl-cdip-50-pixelated-c-with-50-pixelated-dataset-%j.out
...
echo "$(date)"
echo "Starting RVL_CDIP_50_pixelated_C linear class evaluation with 50% pixelated train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir RVL_CDIP_50_pixelated_C/eval/training_124999/linear_class_with_50_pixelated_dataset \

#!/bin/bash
#SBATCH --job-name eval-linear-gender-celeba-masked-a-with-masked-dataset
#SBATCH --output eval-linear-gender-celeba-masked-a-with-masked-dataset-%j.out
...
echo "$(date)"
echo "Starting CelebA_masked_A linear gender evaluation with masked train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir CelebA_masked_A/eval/training_124999/linear_gender_with_masked_train_and_val_dataset \

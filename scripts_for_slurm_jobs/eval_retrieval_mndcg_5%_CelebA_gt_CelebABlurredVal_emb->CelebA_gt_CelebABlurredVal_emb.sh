#!/bin/bash
#SBATCH --job-name eval-retrieval-mndcg-5percent-CelebA_gt_CelebABlurredVal_emb->CelebA_gt_CelebABlurredVal_emb
#SBATCH --output eval-retrieval-mndcg-5percent-CelebA_gt_CelebABlurredVal_emb->CelebA_gt_CelebABlurredVal_emb-%j.out
...
echo "$(date)"
echo "Starting CelebA retrieval mnDCG evaluation for scenario "Unadapted/Blur with Anonymized Query Image" on partition: GPU"

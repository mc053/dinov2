#!/bin/bash
#SBATCH --job-name eval-linear-gender-celeba-gt-with-masked-dataset-2
#SBATCH --output eval-linear-gender-celeba-gt-with-masked-dataset-2-%j.out
...
echo "$(date)"
echo "Starting CelebA_gt linear gender evaluation with masked val dataset and masked train dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir CelebA_gt/eval/training_124999/linear_gender_with_masked_val_dataset_2 \

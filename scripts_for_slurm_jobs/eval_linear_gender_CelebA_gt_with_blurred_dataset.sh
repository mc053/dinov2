#!/bin/bash
#SBATCH --job-name eval-linear-gender-celeba-gt-with-blurred-dataset
#SBATCH --output eval-linear-gender-celeba-gt-with-blurred-dataset-%j.out
...
echo "$(date)"
echo "Starting CelebA_gt linear gender evaluation with blurred train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir CelebA_gt/eval/training_124999/linear_gender_with_blurred_dataset \

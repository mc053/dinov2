#!/bin/bash
#SBATCH --job-name eval-knn-gender-celeba-gt-with-pixelated-dataset-2
#SBATCH --output eval-knn-gender-celeba-gt-with-pixelated-dataset-2-%j.out
...
echo "$(date)"
echo "Starting CelebA_gt knn gender evaluation with pixelated val dataset and pixelated train dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir CelebA_gt/eval/training_124999/knn_gender_with_pixelated_val_dataset_2 \

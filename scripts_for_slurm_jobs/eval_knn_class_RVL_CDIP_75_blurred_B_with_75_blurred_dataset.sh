#!/bin/bash
#SBATCH --job-name eval-knn-class-rvl-cdip-75-blurred-b-with-75-blurred-dataset
#SBATCH --output eval-knn-class-rvl-cdip-75-blurred-b-with-75-blurred-dataset-%j.out
...
echo "$(date)"
echo "Starting RVL_CDIP_75_blurred_B knn class evaluation with 75% blurred train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir RVL_CDIP_75_blurred_B/eval/training_124999/knn_class_with_75_blurred_dataset \

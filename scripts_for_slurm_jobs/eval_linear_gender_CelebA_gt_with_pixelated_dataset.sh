#!/bin/bash
#SBATCH --job-name eval-linear-gender-celeba-gt-with-pixelated-dataset
#SBATCH --output eval-linear-gender-celeba-gt-with-pixelated-dataset-%j.out
...
echo "$(date)"
echo "Starting CelebA_gt linear gender evaluation with pixelated val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir CelebA_gt/eval/training_124999/linear_gender_with_pixelated_val_dataset \

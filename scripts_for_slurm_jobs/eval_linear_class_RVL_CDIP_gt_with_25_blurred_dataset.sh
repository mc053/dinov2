#!/bin/bash
#SBATCH --job-name eval-linear-class-rvl-cdip-gt-with-25-blurred-dataset
#SBATCH --output eval-linear-class-rvl-cdip-gt-with-25-blurred-dataset-%j.out
...
echo "$(date)"
echo "Starting RVL_CDIP_gt linear class evaluation with 25% blurred train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir RVL_CDIP_gt/eval/training_124999/linear_class_with_25_blurred_dataset \

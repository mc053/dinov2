#!/bin/bash
#SBATCH --job-name eval-linear-class-rvl-cdip-50-masked-a-with-50-masked-dataset
#SBATCH --output eval-linear-class-rvl-cdip-50-masked-a-with-50-masked-dataset-%j.out
...
echo "$(date)"
echo "Starting RVL_CDIP_50_masked_A linear class evaluation with 50% masked train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir RVL_CDIP_50_masked_A/eval/training_124999/linear_class_with_50_masked_dataset \

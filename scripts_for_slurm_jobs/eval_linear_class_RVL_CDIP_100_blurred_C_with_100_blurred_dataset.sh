#!/bin/bash
#SBATCH --job-name eval-linear-class-rvl-cdip-100-blurred-c-with-100-blurred-dataset
#SBATCH --output eval-linear-class-rvl-cdip-100-blurred-c-with-100-blurred-dataset-%j.out
...
echo "$(date)"
echo "Starting RVL_CDIP_100_blurred_C linear class evaluation with 100% blurred train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir RVL_CDIP_100_blurred_C/eval/training_124999/linear_class_with_100_blurred_dataset \

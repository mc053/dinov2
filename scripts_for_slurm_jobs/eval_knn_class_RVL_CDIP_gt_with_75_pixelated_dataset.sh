#!/bin/bash
#SBATCH --job-name eval-knn-class-rvl-cdip-gt-with-75-pixelated-dataset
#SBATCH --output eval-knn-class-rvl-cdip-gt-with-75-pixelated-dataset-%j.out
...
echo "$(date)"
echo "Starting RVL_CDIP_gt knn class evaluation with 75% pixelated train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir RVL_CDIP_gt/eval/training_124999/knn_class_with_75_pixelated_dataset \

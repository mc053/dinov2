#!/bin/bash
#SBATCH --job-name eval-knn-class-rvl-cdip-100-masked-a-with-100-masked-dataset
#SBATCH --output eval-knn-class-rvl-cdip-100-masked-a-with-100-masked-dataset-%j.out
...
echo "$(date)"
echo "Starting RVL_CDIP_100_masked_A knn class evaluation with 100% masked train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir RVL_CDIP_100_masked_A/eval/training_124999/knn_class_with_100_masked_dataset \

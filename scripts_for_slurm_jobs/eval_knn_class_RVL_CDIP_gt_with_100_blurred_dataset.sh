#!/bin/bash
#SBATCH --job-name eval-knn-class-rvl-cdip-gt-with-100-blurred-dataset
#SBATCH --output eval-knn-class-rvl-cdip-gt-with-100-blurred-dataset-%j.out
...
echo "$(date)"
echo "Starting RVL_CDIP_gt knn class evaluation with 100% blurred train and val dataset on partition: GPU"
...
srun --unbuffered enroot start --mount $HOME:$HOME/mounted_home -w mc085 bash -c "
    ...
    --output-dir RVL_CDIP_gt/eval/training_124999/knn_class_with_100_blurred_dataset \
